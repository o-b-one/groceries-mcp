...
BRANCH_ID = "2725"
BASE_URL = f"https://www.keshet-teamim.co.il/v2/retailers/{STORE_ID}/branches/{BRANCH_ID}"
...
async def search(item: str) -> dict:
    return await _request(
        url=f"{CATALOG_ENDPOINT}&query={item}",
        method="GET"
    )
...
async def get_cart() -> list[dict]:
    ...
    return [
        types.CartItemSchema(id=str(item['id']), quantity=str(item['quantity'])).model_dump()
